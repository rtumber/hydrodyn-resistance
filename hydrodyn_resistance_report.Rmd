...
```{r dllr_mean_resid_reswei_plot, echo=FALSE, warning=FALSE, message=FALSE}
import_data %>%
  group_by(dlr) %>%
  summarise(res = mean(resid_reswei)) %>%
  ggplot(aes(dlr, res)) +
...
knitr::kable(nlr_rmses)
```

* Froude Number with Prismatic Coefficient

```{r nlr_fn_cp_rmses, echo=FALSE}
knitr::kable(nlr_rmses_cp)
```
...
knitr::kable(rbind((lr_rmses_cp %>% select(-RMSE) %>% rename(RMSE = adjusted_RMSE)), nlr_rmses_cp))
```
