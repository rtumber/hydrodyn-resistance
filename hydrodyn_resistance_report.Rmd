...
The model hyperparameters were tuned using the same method as that for the model containing just Froude number and Prismatic coefficient and as before, the the ranger model required no further tuning.
The tuned RMSEs below the ranger model as being the most accurate so it is this model we will use to predict values on the test set
