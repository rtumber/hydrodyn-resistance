...
```{r lcb_resid_reswei_plot, fig.cap = "Longitudinal position of the center of buoyancy against Residual resistance", echo=FALSE, warning=FALSE}
import_data %>%
  group_by(lcb) %>%
...
```{r cp_resid_reswei_plot, fig.cap = "Prismatic Coefficient against Residual resistance", echo=FALSE, warning=FALSE}
import_data %>%
  group_by(cp) %>%
  summarise(resid_reswei) %>%
...
```{r dlr_resid_reswei_plot, fig.cap = "Length-displacement ratio against Residual resistance", echo=FALSE, warning=FALSE}
import_data %>%
  group_by(dlr) %>%
  summarise(resid_reswei) %>%
...
```{r bt_resid_reswei_plot, fig.cap = "Beam-draft ratio against Residual resistance", echo=FALSE, warning=FALSE}
import_data %>%
  group_by(bt) %>%
  summarise(resid_reswei) %>%
...
```{r lb_resid_reswei_plot, fig.cap = "Length-Beam ratio against Residual resistance", echo=FALSE, warning=FALSE}
import_data %>%
  group_by(lb) %>%
  summarise(resid_reswei) %>%
...
```
The model name was added to the training function to guage progress and identify any potential bottlenecks and the defined cross validation control was added.
##### Linear Regression
...
print(nlr_rmses_cp)
```
In both the linear and non-linear regression models, addition of the prismatic coefficient had either a small rise or a small reduction in the RMSE for the residual resistance, however it is clear a Random Forest approach gives better results.
...
Again, a similar approach to the above in terms of testing and evaluating multiple model types was taken.
...
_split rule_ set as _extratrees_ appeared more successful than variance so was kept as is.
...
The approach consisted of defining the two model types in the same fashion as that for every other modelling operation so far before adding the factor in question to the model containing the Froude number and Primatic coefficient. The lowest RMSE for those models with that factor was then recorded.
To demonstrate, for the Longitudinal position of the center of buoyancy, the operation was run.
```{r rrf_tune, eval=FALSE}
...
```
For all other factors not already included in the model this was repeated, with _lcb_ replaced by the factor.
...
With all the above factors with the exception of the longitudinal position of the center of buoyancy producing negative effects on prediction accuracy the model containing only the factors fn, cp, and lcb will be carried forward and tuned for prediction on the test set.
...
The below table details the calculated RMSEs of the tuned ranger and RRF models for the Froude Number and Prismatic Coefficient and untuned model RMSEs for Froude Numbe, Prismatic Coefficient and each of the addtional factors.
```{r addtional_factor_RMSE_comp_res, echo=FALSE}
exp_factor_RMSEs
```
Looking at these results we can see the improvement in RMSE for both model types with the addition of Longitudinal position of the center of buoyancy, lcb to the models containing Froude Number and Proismatic Coefficient.
...
```
```{r cp_ac_pred_res_resid_plot, fig.cap = "Prismatic Coefficient against Predicted & Actual Residual resistance", echo=FALSE, warning=FALSE}
set.seed(1, sample.kind="Rounding")
res_test_conc %>%
  ggplot() +
...
```
```{r lcb_ac_pred_res_resid_plot, fig.cap = "Longitudinal position of the center of buoyancy against Predicted & Actual Residual resistance", echo=FALSE, warning=FALSE}
set.seed(1, sample.kind="Rounding")
res_test_conc %>%
  ggplot() +
...
```{r res_tst_conc_dif_fn, fig.cap = "Froude Number against Difference between Predicted and Actual Residual Resistance", echo=FALSE, warning=FALSE}
res_test_conc_dif %>%
  ggplot(aes(fn, res_dif)) +
  geom_point()
```
```{r res_tst_conc_dif_cp, fig.cap = "Prismatic Coefficient against Difference between Predicted and Actual Residual Resistance", echo=FALSE, warning=FALSE}
res_test_conc_dif %>%
  ggplot(aes(cp, res_dif)) +
  geom_point()
```
```{r res_tst_conc_dif_lcb, fig.cap = "Longitudinal position of the center of buoyancy against Difference between Predicted and Actual Residual Resistance", echo=FALSE, warning=FALSE}
res_test_conc_dif %>%
